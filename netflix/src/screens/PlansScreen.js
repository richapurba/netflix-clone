...
function PlansScreen() {
	...
	const loadCheckout = async (priceId) => {
		...
		docRef.onSnapshot(async(snap) => {
			...
			if (sessionId) {
				const stripe = await loadStripe("pk_test_51IBkYpJLYiXcTDTowhJ8rao2IvhWcYgfMR65s6ayFjUaKcy368jW5fgnIMkcDI9nj1r0uJXVeurucmLFx84ojRyG00m2jKiVuj");
	...
	return (
		...
		<br />
		{subscription && <p>Renewal date: {new Date(subscription?.current_period_end * 1000).toLocalDateString()}</p>}
